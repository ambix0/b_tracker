...
function render(){
 ...
 $("#thead").innerHTML=`<tr><th>Scheme</th><th>Person</th><th>Type</th>${dates.map(d=>`<th class="center">${esc(fmtShortDate(d))}</th>`).join("")}</tr>`;
 $("#tbody").innerHTML=visible.map(s=>`<tr><td><strong>${esc(s.scheme_name)}</strong></td><td>${esc(s.person_name)}</td><td>${esc(s.person_type)}</td>${dates.map(d=>`<td class="center">${statusHTML(map.get(`${s.id}|${d}`))}</td>`).join("")}</tr>`).join("")||`<tr><td colspan="20" class="center muted">No matching schemes.</td></tr>`;

 $("#cards").innerHTML=visible.map((s,i)=>`<article class="scheme-card">
   <div class="scheme-card-top"><div class="scheme-number">${(page-1)*PAGE_SIZE+i+1}</div><div class="scheme-title"><h3>${esc(s.scheme_name)}</h3><div>${esc(s.person_name)} <span class="dot">·</span> ${esc(s.person_type)}</div></div><span class="chevron">›</span></div>
...
async function load(){
 ...
 [schemes,updates]=await Promise.all([api("schemes?select=id,scheme_name,person_type,person_name&order=scheme_name"),api("bfm_updates?select=scheme_id,update_date,status&order=update_date.desc")]);
 dates=[...new Set(updates.map(x=>x.update_date))].sort((a,b)=>b.localeCompare(a)).slice(0,7).sort();
 const today=todayISO(),todays=updates.filter(x=>x.update_date===today),updated=new Set(todays.filter(x=>x.status).map(x=>x.scheme_id)).size;
 $("#today").textContent=fmtDate(today);$("#total").textContent=schemes.length;$("#updated").textContent=updated;$("#pending").textContent=Math.max(0,schemes.length-updated);$("#completion").textContent=schemes.length?Math.round(updated/schemes.length*100)+"%":"0%";

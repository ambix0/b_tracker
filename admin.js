...
async function refresh(){
 schemes=await api("schemes?select=id,scheme_name,person_type,person_name&order=scheme_name");
 ...
 if(schemes.length)updatePersonFields();else{$("#personName").textContent="—";$("#personType").textContent="—"}
 loadExistingStatus();
 const today=todayISO(),todays=updates.filter(x=>x.update_date===today),u=new Set(todays.filter(x=>x.status).map(x=>x.scheme_id)).size;
 $("#adminToday").textContent=fmtDate(today);$("#aTotal").textContent=schemes.length;$("#aUpdated").textContent=u;$("#aPending").textContent=Math.max(0,schemes.length-u);$("#aCompletion").textContent=schemes.length?Math.round(u/schemes.length*100)+"%":"0%";
...
$("#schemeForm").addEventListener("submit",async e=>{e.preventDefault();const name=$("#newScheme").value.trim(),type=$("#newPersonType").value,person=$("#newPersonName").value.trim();if(!name||!person)return;try{if(editingSchemeId){await api(`schemes?id=eq.${editingSchemeId}`,{method:"PATCH",body:JSON.stringify({scheme_name:name,person_type:type,person_name:person})});$("#schemeMsg").textContent="Scheme updated."}else{await api("schemes",{method:"POST",body:JSON.stringify({scheme_name:name,person_type:type,person_name:person})});$("#schemeMsg").textContent="Scheme added."}resetSchemeForm();await refresh()}catch(err){$("#schemeMsg").textContent="Could not save: "+err.message}});
